...
SELECT MARK, COLOR, COUNT(color) AS 'TOTAL CARS' FROM AUTO
WHERE MARK = 'LADA'
GROUP BY color, mark 
UNION SELECT MARK, COLOR, COUNT(color) AS 'TOTAL CARS' FROM AUTO
WHERE MARK = 'BMW'
GROUP BY color, mark;

-- Задание № 2. Вывести на экран марку авто и количество AUTO не этой марки

SELECT MARK, (COUNT(color) - 1) AS 'TOTAL CARS' FROM AUTO
GROUP BY MARK;
